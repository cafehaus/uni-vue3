      ...
      <view class="HomeLive">
        <view class="HomeLive-title">
          ...
          <view class="HomeLive-titleNum">
            ...
            <text class="HomeLive-titleNumText mark">{{ liveInfo.total }}</text>
        ...
        <view class="HomeLive-content">
          <view v-for="item in roomInfo" :key="item.id" class="NormalRoomItem">
            <view class="NormalRoomItem-show" @click="goLive(item)">
              ...
              <view class="NormalRoomItem-showAnchor">
                ...
                <text class="NormalRoomItem-showAnchorName">{{
                  item.anchor_name
                }}</text>
              </view>
            </view>
            ...
            <view class="NormalRoomItem-title1"
              >时间：{{ item.startTime }}</view
            >
            <view class="NormalRoomItem-footer">
              <text>{{ item.statusName }}</text>
              <text v-if="item.statusName === '已结束'" class="btn"
                >看回放</text
              >
              <text v-if="item.statusName === '还没开始'" class="btn"
                >订阅提醒</text
              >
            </view>
...
export default {
  ...
  methods: {
    ...
    async getLiveInfo() {
      ...
      const res = await this.$api.getLiveInfo()
      ...
      roomInfo = roomInfo.length
        ? roomInfo.map((item) => {
            ...
            item.startTime = this.$util.fmtDate(
              item.start_time * 1000,
              "yyyy-MM-dd hh:mm:ss"
            );
            item.endTime = this.$util.fmtDate(
              item.end_time * 1000,
              "yyyy-MM-dd hh:mm:ss"
            );
            return item;
    ...
    // 跳转直播详情
    goLive(e) {
      let live_status = e.live_status
      let roomid = e.roomid
      let livestatusname = e.statusName
      ...
      if (
        ...
        this.$tips.toast("直播" + livestatusname)
      }
    },
  },
};
